...
if ($#argv != 12) then
        echo "Usage:   		./PhaseC.csh  pdb       name chain model map disulfile ureatype crowdtype RG MG crowdnum box"
        echo "Example: 		./PhaseC.csh  1CFD.pdb  cfd   A    GO    BTmap.dat ds.inp nourea 0 11.485 3250.0 0 300"
	goto done
    endif
set pdb = $argv[1] 
set name = $argv[2]
...
###################   (5) RECALCULATING triple.inp  ###########################

cc triple.c -lm -w -o triple	#compile triple.c
echo $MINIZ $ATOM $PAIR 20 > triple.in
./triple < triple.in
