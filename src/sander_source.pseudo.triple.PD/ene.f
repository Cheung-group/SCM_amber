...
! Qian add,dummy1 is the max id for protein, -83 = -82(types of crowders) -1 (index begins from 0)
   dummy1 = 3*(ntypes-83)
   ecwa = 0.0
! end
   4200 continue
...
! Qian change
      if(jn .ge. dummy2) then
      ecwd = ecwd + (pk(ic)+cosnp*gamc(ic)+sinnp*gams(ic))*fzi(jn)
      endif
! end
#ifdef MPI
      if(decpr .and. (idecomp == 1 .or. idecomp == 2)) then
