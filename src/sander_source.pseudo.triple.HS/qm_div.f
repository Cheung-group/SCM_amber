...
subroutine qm_div(x, ix, f, escf, atom_type)
  ...
  use qmmm_module, only : qmmm_struct, qmmm_nml, allocate_qmmm_pair_list
...
!      qmmm_struct%ipres = ix(i02)
      ...
      do 111 m=1,nres
         ...
         do 120 j=ii,iii
            ...
            if(j == qmmm_nml%iqmatoms(jjpindx)) then !current atm in QM region
               iyes = 1
...
!                      'adjust QM atoms and rerun')
         ...
         120 continue  !end loop through residue
         121 if(iyes == 1) then
         ...
         if(jjp >= qmmm_struct%nquant) goto 131
      111 continue
      131 ntotqmres = k - 1
  endif  !if(idc==2)
...
!Print link atom coords to mdout file
      ...
      qmmm_struct%jqatms(1:qmmm_struct%nquant) = qmmm_nml%iqmatoms(1:qmmm_struct%nquant)
      qmmm_struct%ntotatm = natom
...
!*********distribute link atom forces*************
      do k=1,qmmm_struct%nlink
        ...
        m = (qmmm_nml%iqmatoms(lnk_no)-1)*3
