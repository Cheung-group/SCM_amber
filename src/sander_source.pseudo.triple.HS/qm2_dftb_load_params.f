...
!In parallel all threads call this routine
   ...
   use qm2_dftb_module, only: NDIM, LDIM, MDIM, &     ! Fixed parameters
         ...
         sktab, log_racc, dacc, dispfile, ks_struct, fermi_str
   ...
   ! Dispersion
   character(len=1024) ::  disp_file

   ! CM3 charges
   character(len=1024) ::  cm3_file
...
if (qmmm_mpi%commqmmm_master) then
   ...
   do i = 1, qmmm_struct%nquant_nlink
      mol%atyp( izp_str%izp(i) ) = element_sym( qmmm_struct%iqm_atomic_numbers(i) )
   end do
   ...
   do i = 1,qmmm_struct%nquant_nlink 
      atom_uc=element_sym(qmmm_struct%iqm_atomic_numbers(i))
