...
10 continue
   ...
   if (index(outformat,"short") > 0) then
      call genatlist(natom,nres,igraph,isymbl,ipres,lbres,6,mask)
   else if (index(outformat,"pdb") > 0) then
      title = maskstr  ! print 'maskstr' into a remark field in pdb
      call genpdb(natom,nres,igraph,isymbl,ipres,lbres,6,crd,title,chg,mask)
   else if (index(outformat,"amber") > 0) then
      call gengroupinput(natom,nres,igraph,isymbl,ipres,lbres,6,mask)
   else
...
subroutine getnam(natom,nres,igraph,isymbl,ipres,lbres,ititl,nf,chg)
   ...
   integer i6dmax, edmax, idmax, ntype, nttyp, astat
...
subroutine genatlist(natom,nres,igraph,isymbl,ipres,lbres,nf,mask)
...
subroutine genpdb(natom,nres,igraph,isymbl,ipres,lbres,nf,crd,title,chg,mask)
...
subroutine gengroupinput(natom,nres,igraph,isymbl,ipres,lbres,nf,mask)
