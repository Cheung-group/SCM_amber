...
subroutine allocate_gb( natom,ncopy )
   ...
   allocate( r2x(ncopy*natom), rjx(ncopy*natom), vectmp1(ncopy*natom), vectmp2(ncopy*natom),   &
             ...
             longskipv(natom*ncopy), nradii(natom),lesscalefac(natom*ncopy), rbornlong(natom*ncopy), &
             rix(natom*ncopy),iridx(natom*ncopy),jridx(natom*ncopy),vtemp7(ncopy),stat = ier )
#else
...
#ifdef LES
   use pimd_vars, only: ipimd,ineb,nrg_all
#  ifdef MPI
      use remd, only : rem
#  endif
#endif
      ...
      if (qmmm_nml%qmgb == 1) then
        ...
        do qm_temp_count = 1, qmmm_struct%nquant
           charge(qmmm_nml%iqmatoms(qm_temp_count)) = &
               qmmm_struct%qm_resp_charges(qm_temp_count)
   ...
   do i=1,maxi
      ...
      if (qmmm_nml%ifqnt) then
         if ( qmmm_struct%atom_mask(i) ) then  !yes it is
           ...
           do qm_temp_count = 1, qmmm_struct%nquant
              skipv(qmmm_nml%iqmatoms(qm_temp_count)) = .true.
      ...
      nnbips = nnbips + 1    ! count the self interaction
      icount = 0
      do j=i+1,natom
...
!!! !dir$ ivdep
      ...
      do k=1,icount
         ...
         if( igb/=6 ) then
           ...
           e = -qiqj*dl*(fgbi + one_Arad_beta)*scalefac(k)*lesscalefac(k)
#else
      ...
      do i=1,maxi
         ...
         if (icnum.eq.0) then
           ...
           do k=1,ncopy
              ...
              qi2h = half*qi*qi
              qid2h = qi2h*dl
              self_e = self_e + qid2h*(onereff(istrt+k) + one_Arad_beta) *lfaci
           ...
           enddo
            ...
            qi2h = half*qi*qi
            qid2h = qi2h*dl
            self_e =  qid2h*(onereff(istrt+icnum) + one_Arad_beta) *lfac
         ...
         self_e = qid2h*(onereff(i) + one_Arad_beta)
...
#  endif
         ...
         if( igb == 2 .or. igb == 5 .or. igb == 7) then
         ...
         do k=1,icount
            ...
            if (dij <= rgbmax +sj) then
              ...
              if( igb == 7 .and. dij < rborn(i) +rborn(j) + GBNECKCUT) then
              ...
              end if !f( igb == 7 .and. dij < rborn(i) +rborn(j) + GBNECKCUT)
              ...
              if( igb == 2 .or. igb == 5 .or. igb == 7) datmp = datmp*thi2
              f_x = xij*datmp
              f_y = yij*datmp
              f_z = zij*datmp
              if (qmmm_nml%ifqnt) then
...
#ifdef LES
      if(ipimd>0.or.ineb>0) nrg_all(icnum) = nrg_all(icnum) + nrg_egb_tmp
#endif
...
!======== QMMM ==========
   if (qmmm_nml%ifqnt) then
     ...
     if (qmmm_nml%qmgb /= 0) then
        ...
        ! correctly skipped for QM-MM on the next step.
        do qm_temp_count = 1, qmmm_struct%nquant_nlink
           charge(qmmm_nml%iqmatoms(qm_temp_count)) = zero
        end do
     end if
     ...
     do i=1,qmmm_struct%nlink
       ...
       j = (qmmm_nml%iqmatoms(lnk_no)-1)*3
...
#ifdef LES
       if(ipimd>0.or.ineb>0) nrg_all(icnum) = nrg_all(icnum) + nrg_vdw_tmp
#endif
...
! apply to multiple atoms for LES
         ...
         if (dij <= rgbmax +si) then
           ...
           if (igb == 7 .and. dij < rborn(j) +rborn(i) +GBNECKCUT) then 
