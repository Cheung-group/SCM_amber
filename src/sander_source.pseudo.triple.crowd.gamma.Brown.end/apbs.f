! $Id: apbs.f,v 1.3 2007/10/12 22:01:36 rok Exp $
!
#include "dprec.h"
...
#include "files.h" 
    ...
    INTEGER :: calcnpenergy, wpot, wchg, wsmol
    ...
    NAMELIST /apbs/ dime, pdime, cglen, fglen, grid, nlev, &
         ...
         temp, gamma, sdens, calc_type, calcnpenergy, &
...
!   i_pbeparm(1) = 1 ! molid
    ...
    i_param(13) = wchg
    i_param(14) = wsmol
    i_param(15) = usedielmap
    i_param(16) = dielmapid
    i_param(17) = usekappamap
    i_param(18) = kappamapid
    i_param(19) = usechargemap
    i_param(20) = chargemapid
...
! boundary conditions, bcfl
    ...
    ELSE IF (i_param(8) == 1) THEN
       WRITE(6, '(a)') 'Single Debye-Hukel boundary conditions'
    ELSE IF (i_param(8) == 2) THEN
       WRITE(6, '(a)') 'Multiple Debye-Huckel boundary conditions'
...
! surface definition, srfm
    ...
    ELSE IF (i_param(9) == 2) THEN
       WRITE (6, '(a)') 'Cubic-spline surface'
    ELSE IF (i_param(9) == 3) THEN
       WRITE (6, '(a)') '7-order polynomial spline surface'
    ELSE
...
! charge discretization, chgm
    ...
    ELSE IF(i_param(6) == 1) THEN
       WRITE(6, '(a)') 'Using cubic B-spline discretization'
    ELSE IF(i_param(6) == 2) THEN
       WRITE(6, '(a)') 'Using quintic B-spline discretization'
    ELSE
...
!           fglen(2)/dime(2), fglen(3)/dime(3)
   ...
   ELSE IF (radiopt == 2) THEN
      WRITE(6, '(a)') 'Using charge/radii information form PQR file'
   ELSE IF (radiopt == 3) THEN
      WRITE(6, '(a)') 'Using radii information form PQR file'
   ELSE
