...
#  include "pb_md.h"
   ...
   namelist /pb/epsin, epsout, istrng, pbtemp, radiopt, sprob, maxsph, &
      maxarc, arcres, smoothopt, fillratio, space, nbuffer, accept, maxitn, &
   ...
   npopt = 2
   maxsph   = 400
   !maxarcdot= 500 ! HG Set this to smaller values (e.g., 500) for larger systems
   maxarcdot= 1500
   maxarc   = 256
   nbuffer = 0
   sprob  = 1.6d0
   ...
   maxarcdot = maxarcdot*nint(0.5d0/arcres)
   ...
   if ( max(cutnb,cutsa,cutfd) > cutres ) then
      write(6, *) 'PB Bomb in pb_read(): cutnb/cutfd/cutsa must be <= cutres', cutnb, cutfd, cutsa, cutres
      call mexit(6, 1)
   endif
...
#  include "pb_md.h"
   ...
   write(6, *)
   write(6, '(a,i5,a)') 'Do FDPB every', ndofd, ' steps'
   write(6, *) 'Nonbonded Update'
   write(6, '(a, f8.3)' ) '  residue cutoff is set to   ', sqrt(cutres)
   write(6, '(a, f8.3)' ) '  fdpb cutoff is set to      ', sqrt(cutfd)
   write(6, '(a, f8.3)' ) '  sas cutoff is set to       ', sqrt(cutsa)
   write(6, '(a, f8.3)' ) '  nonbonded cutoff is set to ', sqrt(cutnb)
   write(6, *) 'Grid Constants'
   write(6, '(a,3i5)') '  Grid dimensions: ', xm, ym, zm
   write(6, '(a,f8.3)' ) '  Grid spacing set to ', h
   ...
   if ( ifcap == 1 .or. ifcap == 2 ) then
      ...
      if ( ifcap == 3 ) then
         write(6, *) '  A spherical dielectric boundary is used for GSBP and '
      else if ( ifcap == 4 ) then
         write(6, *) '  An orthorhombic dielectric boundary is used for GSBP and '
      else if ( ifcap == 5 ) then
         write(6, *) '  A shell-like dielectric boundary is used and '
      end if
      if ( radiopt == 0 ) then
         write(6, *) '  Cavity radii in the prmtop file are used'
      else if ( radiopt == 1 ) then
         write(6, *) '  Tan, Yang, and Luo optimized cavity radii definition is used'
      else if ( radiopt == 2 ) then
         write(6, *) '  Pymol radii definition for potential surface display is used'
      else
   ...
   if ( srsas ) then
      ...
      write(6, '(a,i5,a)' ) '    Compute SAS every ', ndosas, ' steps'
      write(6, '(a,f8.3)' ) '    Solvent probe radius:       ', sprob
      write(6, '(a,i5)' )   '    Surface dots per atom       ', maxsph
      write(6, '(a,f8.3)' ) '    Buried atom radii increment ', radinc
      write(6, '(a,f8.3)' ) '    Threshhold for exposed atom ', expthresh
      write(6, '(a,f8.3)' ) '    Current SAS                 ', prtsas
   else
   ...
   write(6, '(a,f8.3)' ) '  Solute dielectric constant  :', epsin/eps0
   write(6, '(a,f8.3)' ) '  Solvent dielectric constant :', epsout/eps0
   write(6, '(a,f8.3)' ) '  Temperature (K)             :', pbtemp
   write(6, '(a,f8.3)' ) '  Ionic strength (mM)         :', fiono*istrng
   write(6, '(a,f8.3)' ) '  Debye-Huckel parameter (1/A):', pbkappa
   write(6, *)
   ...
   write(6, '(a,i5)') '  Maximum iterations  :', maxitn
   write(6, '(a,f8.3)') '  Convergence criteria:', accept
...
#include "box.h"
   ...
   ! deallocate ibelly and outflag

   if ( ifcap == 3 .or. ifcap == 4 ) then
      deallocate( ibelly, stat = alloc_err(30) )
   end if
   ...
   if ( alloc_err( 1)+alloc_err( 2)+alloc_err( 3)+alloc_err( 4)+alloc_err( 5)+&
      ...
      write(6, *) 'PB Bomb in pb_free(): Deallocation aborted', alloc_err(1:31)
      call mexit(6, 1)
   end if
...
subroutine pb_init(ifcap,natom,nres,ntypes,nbonh,nbona,ipres,iac,ico,numex,natex, &
                   ibh,jbh,iba,jba,ibel,lbres,igraph,isymbl,cg,rin)
...
#  include "pb_md.h"
   ...
   ! The "maxmax > 0" in the following is to "overcome" int overflow if
   !    (natom*natom/2+1) > 2^31-1
   if ( maxmax > 0 .and. maxnba > maxmax ) maxnba = maxmax 
   if ( maxmax > 0 .and. maxnbr > maxmax ) maxnbr = maxmax
