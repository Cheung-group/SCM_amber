...
!dEVB   use evb_parm,  only: xch_dcrypt
   ...
   use schlegel,  only: ndg, xdat_xdg, xdat_ts, xdat_min, alpha, dg_weight &
                      ...
                      , nUFF, nUFF_dcrypt, xvdw, dvdw, zvdw, Avdw, Bvdw
...
#  include "files.h"
   ...
   integer :: n, nn, io, ios, alloc_error

   integer :: ndia_shift, nxch_cnst, nxch_exp, nxch_gauss &
            , nmin_xfile, nts_xfile, ndg_xfile, nlambda &
            , nxch_dg
   integer :: xch_check(4), dyn_check(4), dia_check(1)
   _REAL_  :: dgpt_alpha(xdg_size), dgpt_weight(xdg_size)
   _REAL_  :: gauss_weight(3)

   namelist / evb / nevb, nbias, nmorse, nmodvdw, nUFF, dia_type, xch_type &
                  , evb_dyn, debug_toler, ioe, dia_shift, xch_cnst, xch_exp &
                  , xch_gauss, morsify, modvdw, UFF, emap, egap_umb, dbonds_umb &
...
! #+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#
   ...
   use evb_parm,  only: evb_dcrypt, C_evb, C_xch &
                      ...
                      , nmorse, nmodvdw, nbias, dia_type, xch_type, evb_dyn &
                      , dbonds_RC, bond_RC
!dEVB   use evb_parm,  only: xch_dcrypt
   use evb_amber, only: morse, k_harm, r0_harm, vdw_mod
   use schlegel,  only: nUFF, nUFF_dcrypt
