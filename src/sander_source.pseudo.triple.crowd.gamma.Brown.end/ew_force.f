...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+
subroutine ewald_force(crd,numatoms,iac,ico,nntypes,charge, &
   ...
   use qmmm_module, only : qmmm_struct, qm2_struct, qmmm_nml, qmewald, &
                           qmmm_scratch
   use parms, only : charmm
#ifdef LES
   use pimd_vars, only : ipimd, ineb, nrg_all,nbead
        ...
        if(i_do_recip)then
           ...
           do i=1,qmmm_struct%nlink
             ...
             lnk_no = qmmm_struct%link_pairs(2,i) !Nquant number of QM atom 
                                                  !      bound to link atom
             qm_no = qmmm_nml%iqmatoms(lnk_no)
           ...
           if (qmmm_nml%qm_pme) then
             ...
             qmmm_scratch%qm_real_scratch(1:3*natom) = zero
             do qm_temp_count = 1, qmmm_struct%nquant_nlink
                qmmm_scratch%qm_pme_scratch(qmmm_nml%iqmatoms(qm_temp_count)) &
                     = qm2_struct%scf_mchg(qm_temp_count)*AMBER_ELECTROSTATIC
             end do
             ...
             do qm_temp_count = 1, qmmm_struct%nquant
               i = qmmm_nml%iqmatoms(qm_temp_count)
               frc(1,i)=frc(1,i)-qmmm_scratch%qm_real_scratch(3*i-2)
               frc(2,i)=frc(2,i)-qmmm_scratch%qm_real_scratch(3*i-1)
               frc(3,i)=frc(3,i)-qmmm_scratch%qm_real_scratch(3*i)
             end do
...
!Antonios add
        call get_14_cg_antonis(charge,crd,frc,iac,ico,ntypes, &
              cn1,cn2, ee14,enb14,scee,scnb, &
...
!Antonios end
   ...
   if (nlesadj > 0.and.ipimd==0.and.ineb==0) then
   ...
   if ( mpoltype == 0 )then
      if (charmm) then
        call get_14_cg(charge,crd,frc,iac,ico,ntypes, &
              cn114,cn214, ee14,enb14,scee,scnb, &
              e14vir,ix,commsander_mytaskid,commsander_numtasks,eedmeth)
      else
        call get_14_cg(charge,crd,frc,iac,ico,ntypes, &
              cn1,cn2, ee14,enb14,scee,scnb, &
      ...
      call get_14_dipole(charge,crd,frc, &
            x(linddip),x(lfield), &
            iac,ico,ntypes, &
            cn1,cn2, ee14,enb14,epol14,scee,scnb,e14vir,ix, &
...
!AS mpi-debug write(72,*)evdw Partial output
...
!write(72,*)rl_temp(1)
#  endif
!AS mpi-debug write(72,*)evdw Total output
!write(72,*)"ew_force",echi 
#  ifdef LES
#   ifdef USE_MPI_IN_PLACE
...
!AS mpi-debug write(72,*)evdw Total output
   ...
   evdw = evdw + evdwr 
!AS mpi-debug      write(72,*)evdw Output is total
#ifdef LES
...
!   if(ipimd>0.or.ineb>0) nrg_all(:) = nrg_all(:) + evdwr/ncopy
   if( ( ipimd > 0 .or. ineb > 0 ) .and. master ) &
   ...
   if ( issetup == 0 )then
      ...
      do i = 1,numatoms
         ...
         if(idecomp == 1 .or. idecomp == 2) then
            ...
            call decpair(2,i,i,-cg(i)*cg(i)*ewaldcof*INVSQRTPI)
         endif
#endif
      end do
      ! -- ti decomp
      ...
      do i=1,numatoms
         ...
         if(cnum(i).eq.0) then
            nrg_all(1:nbead)=nrg_all(1:nbead) &
                 - lfac*cg(i)*cg(i)*ewaldcof*INVSQRTPI*nbead_inv
         else 
            nrg_all(cnum(i)) = nrg_all(cnum(i)) &
                 - lfac*cg(i)*cg(i)*ewaldcof*INVSQRTPI
         endif
      end do
   end if
#endif
