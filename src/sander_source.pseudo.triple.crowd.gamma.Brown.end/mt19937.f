  ...
  function random_int32(self) result( y )
    ...
    if ( self%mti > n ) then ! generate N words at one time
      ...
      do kk = 1, n-m, 1
        ...
        if ( btest( y, 0 ) ) then
          self%mt(kk) = ieor( ieor( self%mt(kk+m), ishft( y, -1 ) ), matrix_a )
        else
          self%mt(kk) = ieor( ieor( self%mt(kk+m), ishft( y, -1 ) ), matrix_b )
        end if
      end do
      do kk = n-m+1, n-1, 1
        y = ibits( self%mt(kk+1), 0, 31 )
        call mvbits( self%mt(kk), 31, 1, y, 31 )
        if ( btest( y, 0 ) ) then
          self%mt(kk) = ieor( ieor( self%mt(kk+m-n), ishft( y, -1 ) ), matrix_a )
        else
          self%mt(kk) = ieor( ieor( self%mt(kk+m-n), ishft( y, -1 ) ), matrix_b )
        end if
      end do
      y = ibits( self%mt(1), 0, 31 )
      call mvbits( self%mt(n), 31, 1, y, 31 )
      if ( btest( y, 0 ) ) then
        self%mt(kk) = ieor( ieor( self%mt(m), ishft( y, -1 ) ), matrix_a )
      else
        self%mt(kk) = ieor( ieor( self%mt(m), ishft( y, -1 ) ), matrix_b )
      end if
      self%mti = 1_wi
    end if
    y = self%mt(self%mti)
