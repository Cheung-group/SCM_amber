...
subroutine pb_force( natom,nres,ntypes,npdec,ipres,iac,ico,natex,cn1,cn2,cg,x,f,enb,eel,eelrf )
   ...
   use solvent_accessibility, only : radi, radip, radip2, radip3, nzratm, &
...
#  include "box.h"
   ...
   integer iatm, proatm, atmfirst, atmlast, i
   ...
   if ( ntnbr == 1 ) then
      if( mpopt == 1 ) then
         ...
         call pb_atmpart2(pbverbose,pbprint,natom,ibgwat,ienwat,ibgion,ienion, &
                          inatm,outwat,oution,ipres,outflag, &
                          cutcap,x,ifcap)                           ! Use cutcap here, not pbcutcap because
                                                                    !    the latter is augmented by TWO
      end if
   end if
   ...
   if ( srsas .and. ( ifcap == 0 .or. ifcap == 5 ) ) then
      if( ifcap == 5 ) then
         call sa_init(pbverbose,pbprint,natom,inatm,ifcap,radi,radip,radip2,outflag)
         call sa_driver(pbverbose,pbprint,natom,inatm,dosas,ndosas,npbstep,nsaslag,&
                        acrd(1,1),iar1pb(1,0),iprshrt,nex,iex)
      else
         call sa_init(pbverbose,pbprint,natom,natom,ifcap,radi,radip,radip2,outflag)
         call sa_driver(pbverbose,pbprint,natom,natom,dosas,ndosas,npbstep,nsaslag,&
                        acrd(1,1),iar1pb(1,0),iprshrt,nex,iex)
      end if
   end if
   call timer_stop(TIME_PBSETUP)
   ...
   if ( epsout /= epsin .and. mpopt /= 2 ) then
      ...
      if( ifcap == 2 .or. ifcap == 5 ) then
         call pb_fdfrc(pbverbose,pbprint,pbgrid,ifcap,natom,inatm,npdec,idecomp,irespw, &
                       ipres,jgroup,ibgwat,ibgion,pbfrc,eelrffd)
      else
         call pb_fdfrc(pbverbose,pbprint,pbgrid,ifcap,natom,natom,npdec,idecomp,irespw, &
                       ipres,jgroup,ibgwat,ibgion,pbfrc,eelrffd)
      end if
   end if
   ...
   if ( cutnb == ZERO ) then
      call pb_directnocut(natom,inatm,ipres,ibgwat,ienwat,ibgion,ienion,ntypes,dbfopt,idecomp,ifcap, &
...
subroutine pb_fdfrc( pbverbose,pbprint,pbgrid,ifcap,natom,atmlast,npdec,idecomp,irespw, &
                     ipres,jgroup,ibgwat,ibgion,pbfrc,eelrf )

   ! passed variables
    
   logical pbverbose, pbprint, pbgrid  
   integer ifcap, natom, atmlast, npdec, idecomp, ibgwat, ibgion
   ...
   do m = 1, mpdec
      ...
      do level = 1, nfocus
         ...
         ! when ifcap == 0 or  == 5, do dielectric map assignment everystep
         ! when ifcap /= 0 and /= 5, do dielectric map assignment once only when grid is set up

         call timer_start(TIME_PBEPS)
         if ( ifcap == 0 .or. ifcap == 5 .or. pbgrid ) call pb_exmol( pbverbose,ifcap )
         ...
         call pb_fddrv( 1,atmlast,phi,xs(xsoffset),sbv )
         ...
         if ( pbverbose .and. pbprint ) then
            call pb_print( ifcap )
            write(6, '(a,i5)' ) '  Iterations required        :', itn
            write(6, '(a,e13.5)' ) '  Norm of the constant vector:', inorm
            write(6, '(a,e13.5)' ) '  Norm of the residual vector:', norm
            write(6, '(a,e13.5)' ) '  Convergence achieved       :', norm/inorm
            write(6, *)
...
subroutine pb_dbene( verbose,eneout,natom,atmlast,ifcap,npdec,idecomp,m,irespw,ipres, &
   ...
   use constants, only: TWOPI, INV_AMBER_ELECTROSTATIC2
