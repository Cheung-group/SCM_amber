...
!   so I include a pair in the 1-4 list if kdiv is > 0 and so is FMN(ic0)
...
#  include "memory.h"
   ...
   REQUIRE( ier==0 )
   allocate ( i11(2*(natom+numextra)), stat=ier)
   REQUIRE( ier==0 )
   allocate ( i12(6*(nbonh+nbona+nbper)), stat=ier)
   REQUIRE( ier==0 )
   ...
   !      ---re-use i14) and then copy to permanent
   
   call build_14nb(i14,numnb14,max14, &
         iph,jph,kph,lph,icph,ipa,jpa,kpa,lpa,icpa, &
         nphih,nphia,ndper,fmn, &
         epowner,numnghbr,enghbrs, &
         natom,offset,test,s3, &
         chngmask,verbose)
   call copy_14nb(i14,ix(inb_14),numnb14)
...
subroutine build_14nb(nb14,numnb14,maxnb14, &
      iph,jph,kph,lph,icph,ipa,jpa,kpa,lpa,icpa, &
      nphih,nphia,ndper,fmn, &
      epowner,numnghbr,enghbrs, &
      natom,scr1,scr2,scr3, &
      chngmask,verbose)
   implicit none

   integer nb14(2,*),numnb14,maxnb14, &
         iph(*),jph(*),kph(*),lph(*),icph(*), &
         ipa(*),jpa(*),kpa(*),lpa(*),icpa(*), &
         nphih,nphia,ndper,chngmask,verbose
   integer enghbrs(5,*),numnghbr(3,*),epowner(*)
   integer natom,scr1(*),scr2(*),scr3(*)
   _REAL_ fmn(*)
   integer ifail,n
   numnb14 = 0
   call do_14pairs(iph,jph,kph,lph,icph,fmn, &
         nphih,nb14,numnb14,maxnb14, &
         epowner,numnghbr,enghbrs,ifail,chngmask)
   if ( ifail == 1 )then
      write(6,*)'exceeded maxnb14 in build14: check extra_pts.h'
      call mexit(6,1)
   end if
   call do_14pairs(ipa,jpa,kpa,lpa,icpa,fmn, &
         nphia+ndper,nb14,numnb14,maxnb14, &
         epowner,numnghbr,enghbrs,ifail,chngmask)
   if ( ifail == 1 )then
      write(6,*)'exceeded maxnb14 in build14: check extra_pts.h'
      call mexit(6,1)
   end if
   call sort_pairs(nb14,numnb14,natom,scr1,scr2,scr3)
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+
subroutine get_14_cg_antonis(charge,crd,frc, &
      iac,ico,ntypes,cn1,cn2, &
      ee14,enb14,scee,scnb,e14vir,ix,mytaskid,numtasks,eedmeth)
   implicit none
   _REAL_ charge(*),crd(3,*),frc(3,*), &
         cn1(*),cn2(*),e14vir(3,3)
   integer iac(*),ico(*),ntypes,ix(*)
   _REAL_ ee14,enb14,scee,scnb
   integer mytaskid,numtasks,eedmeth
#  include "extra_pts.h"
   call do_14_cg_antonis(charge,crd,frc, &
         iac,ico,ntypes,cn1,cn2, &
         ee14,enb14,scee,scnb,e14vir,ix(inb_14),numnb14, &
         mytaskid,numtasks,eedmeth)
   return
end subroutine get_14_cg_antonis 
! Antonios end:
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+
subroutine do_14_cg_antonis(charge,crd,frc, &
      iac,ico,ntypes,cn1,cn2, &
      ee14,enb14,scee,scnb,e14vir,nb14,numnb14, &
      mytaskid,numtasks,eedmeth)
  ...
  use softcore, only: ifsc, nsc, sc_ener, oneweight
   ...
   _REAL_ charge(*),crd(3,*),frc(3,*), &
         cn1(*),cn2(*),e14vir(3,3)
   integer iac(*),ico(*),ntypes,nb14(2,*),numnb14
   _REAL_ ee14,enb14,scee,scnb
   integer mytaskid,numtasks,eedmeth
#  include "flocntrl.h"
#  include "md.h"
...
!Antonios end 
   integer n,i,j,ic
   _REAL_ dx,dy,dz,r2,r2inv,rinv
   _REAL_ scee0,scnb0,g,f6,f12,r6,df
   ee14 = zero
   enb14 = zero
!#ifdef MPI /* SOFT CORE */
!   sc_ener(4) = 0.0d0
...
!         i = nb14(1,n)
!         j = nb14(2,n)
         delx_ixi_ixj(n) = crd(1,NXI(n)) - crd(1,NXI1(n))
         dely_ixi_ixj(n) = crd(2,NXI(n)) - crd(2,NXI1(n))
         delz_ixi_ixj(n) = crd(3,NXI(n)) - crd(3,NXI1(n))
         delx_ixj_ixk(n) = crd(1,NXI2(n)) - crd(1,NXI1(n))
         dely_ixj_ixk(n) = crd(2,NXI2(n)) - crd(2,NXI1(n))
         delz_ixj_ixk(n) = crd(3,NXI2(n)) - crd(3,NXI1(n))
         delx_ixk_ixl(n) = crd(1,NXI3(n)) - crd(1,NXI2(n))
         dely_ixk_ixl(n) = crd(2,NXI3(n)) - crd(2,NXI2(n))
         delz_ixk_ixl(n) = crd(3,NXI3(n)) - crd(3,NXI2(n))
!         write(72,*)n,NXI(n),NXI1(n),crd(1,NXI(n)),crd(1,NXI1(n))
!         write(72,*)crd(1,1)
      end do
      do n = 1,ICHI
...
!         write(72,*)delx_xcx(n),dely_xcy(n),delz_xcz(n)
      end do
      do n = 1,108
!      write(72,*)crd(1,n),crd(2,n),crd(3,n)
      end do
         !r2 = dx*dx + dy*dy + dz*dz
         !rinv = sqrt(1.d0/r2)
         !r2inv = rinv*rinv
...
!      end do  !  n = 1,numnb14
   ...
   return
end subroutine do_14_cg_antonis 
!Antonios end
!--------------------------------------------------------------
!       DO_14_CG
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
subroutine get_14_dipole(charge,crd,frc, &
      ...
      ee14,enb14,epol14,scee,scnb,e14vir,ix, &
      mytaskid,numtasks)
   implicit none
   _REAL_ charge(*),crd(3,*),frc(3,*), &
         cn1(*),cn2(*),e14vir(3,3)
   ...
   integer iac(*),ico(*),ntypes,ix(*)
   _REAL_ ee14,enb14,scee,scnb,epol14
...
#  include "ew_cntrl.h"
   ...
   call do_14_dipole(charge,crd,frc,dipole,field, &
         iac,ico,ntypes,cn1,cn2, &
         ee14,enb14,epol14,scee,scnb,scdd, &
         e14vir,ix(inb_14),numnb14,mytaskid,numtasks)
   return
...
subroutine do_14_dipole(charge,crd,frc,dipole,field, &
      iac,ico,ntypes,cn1,cn2, &
      ee14,enb14,epol14,scee,scnb,scdd, &
      e14vir,nb14,numnb14,mytaskid,numtasks)
   implicit none
   _REAL_ charge(*),crd(3,*),frc(3,*), &
         cn1(*),cn2(*),e14vir(3,3)
   _REAL_ dipole(3,*),field(3,*)
   integer iac(*),ico(*),ntypes,nb14(2,*),numnb14
   _REAL_ ee14,enb14,scee,scnb,scdd,epol14
   integer mytaskid,numtasks
#  include "flocntrl.h"
...
#  include "memory.h"
   ...
   i_ptr = lasti
   call adj_mem_ptr(i_ptr,inb_14, 2*numnb14)
   lasti = i_ptr
   return
...
subroutine copy_14nb(from14,to14,numnb14)
   implicit none
   integer from14(2,*),to14(2,*),numnb14
   integer n
   do n = 1,numnb14
      to14(1,n) = from14(1,n)
      to14(2,n) = from14(2,n)
...
subroutine do_14pairs(ip,jp,kp,lp,icp,fmn, &
      nphi,list,num,maxnb14, &
      epowner,numnghbr,enghbrs,ifail,chngmask)
   use qmmm_module, only : qmmm_nml,qmmm_struct
   implicit none
   integer ip(*),jp(*),kp(*),lp(*),icp(*)
   integer nphi,list(2,*),num,maxnb14,ifail
   integer enghbrs(5,*),numnghbr(3,*),epowner(*),chngmask
   _REAL_ fmn(*),f
   integer i,j,k,l,ii,ll,ic0,n,ni,nl,i3,j3,k3,l3
   logical qmmm_skip
   ifail = 0
   do n = 1,nphi
      i3 = ip(n)
