...
! use trapezoidal rule for each integral
  do j = 1,num_vdw_atom_types
    do i = 1,num_vdw_atom_types
      ...
      ene_vdw_longrange_factor = ene_vdw_longrange_factor + two*pi* &
                 vdw_type_count(i)*vdw_type_count(j)*sume
      vir_vdw_longrange_factor = vir_vdw_longrange_factor + two*pi* &
...
subroutine AM_VDW_DIRECT_ene_frc_i(i,ipairs,numtot,xk,yk,zk, &
   ...
   integer :: itran,it,jt,ih,jh,j,m,np,mask27
   _REAL_ :: xktran(3,18)
   _REAL_ :: wi,wj,xi,yi,zi,delx,dely,delz,delr2,delr,eps,rad
   _REAL_ :: rho,t1,t2,dt1drho,dt2drho,drhodr,term,dfx,dfy,dfz,cut2
   _REAL_ :: rho6,rho7
