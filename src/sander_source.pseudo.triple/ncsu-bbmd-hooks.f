...
#  include "ncsu-mpi.h"
   ...
   call ctxt_on_force(contexts(current), x, f, virial, mdstep)
...
1  call mpi_bcast(exchange, 1, MPI_INTEGER, 0, commsander, error)
   ...
   if (exchange.eq.1) then
      ...
      if (sanderrank.eq.0) then
         write (unit = OUT_UNIT, fmt ='(a,a,'//pfmt(partner)//')') &
            NCSU_INFO, 'BBMD : exchanged coordinates/velocities with ', partner
