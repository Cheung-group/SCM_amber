...
#include "mpif.h"
...
#endif
  _REAL_,save :: scalpha                                     ! parameter alpha in the SC-potential
  integer, save :: ifsc
  ...
  _REAL_,save :: oneweight, sc_ener(8)                       ! inverse of the lambda-weight, energies of softcore parts
  _REAL_,save :: sc_ener_ave(8), sc_ener_rms(8)
  _REAL_,save :: sc_ener_tmp(8), sc_ener_tmp2(8), sc_ener_old(8), sc_ener_old2(8) ! for running averages
...
!===================================================================================================
    ...
    subroutine setup_sc(natom,nres,igraph,isymbl,ipres, &
      ...
      if (sanderrank==0) then
         write(6,*) '      '
         write(6,'(a,a,a,i5,a)')  '     Softcore Mask ', scmask(1:len_trim(scmask)), ' matches ',nsoftcore,' atoms'
      end if
      ...
      if (sanderrank==0 .and. ifsc /=2 ) then
         ...
         if (ier_alloc /= 0) call sander_bomb('allocate_arrays[softcore.f]','cant allocate nsc_partner','')
         ...
         if (nmixed /= natom_partner-nsoftcore_partner) then
            call sander_bomb('setup_sc','All non-softcore atoms must be identical in both systems','')
         end if
      ...
      sc_ener_tmp(1:8) = 0.0d0
      sc_ener_tmp2(1:8) = 0.0d0
    ...
    subroutine sc_degrees_o_freedom(ndfmin)
      ...
      if ( nsoftcore > 0 ) then
         ...
         if (sc_dof_shaked>0) then
            ...
            if (sanderrank==0) write(6,'(a,i4,a,i4)')  '   DOF for the SC part of the system: ', sc_deg, &
                 ' SHAKE constraints in the SC region: ', sc_dof_shaked
         end if
...
!===================================================================================================
    ...
    subroutine sc_allocate_arrays(natom, ntypes, nstlim)
      ...
      if (logdvdl ==1) then
         ...
         if (ier_alloc /= 0) call sander_bomb('allocate_arrays[softcore.f]','cant allocate dvdl_values','')
      end if
...
!===================================================================================================
    ...
    subroutine build_force_masks(natom)
      ...
      if (ier_alloc /= 0) call sander_bomb('abuild_force_masks[softcore.f]','cant allocate masks','')
...
!===================================================================================================
    ...
    subroutine sc_check_and_adjust_crd(natom, crd)
      ...
      if (nadj>9) then
         write (6,'(a,i5,a)') '     A total of ',nadj,' small coordinate adjustments were made, check results carefully.'
      end if
...
!===================================================================================================
    ...
    subroutine sc_check_perturbed_molecules(nummols, molsiz)
      ...
      if (ier_alloc /= 0) call sander_bomb('sc_check_perturbed_molecules[softcore.f]','cant allocate molecule_type','')

      allocate (molecule_type_partner(nummols_partner),stat=ier_alloc)
      if (ier_alloc /= 0) call sander_bomb('sc_check_perturbed_molecules[softcore.f]','cant allocate molecule_type_partner','')
      ...
      do imol = 1, nummols
         if (molecule_type(imol) == 2) then
            ...
            if (molecule_type(imol) == 0 .and. molecule_type_partner(i) == 1) then
               ...
               molecule_type(imol) = 1
               write (6,'(a,i5,a)') '     Molecule ',imol, &
               ' is nonsoftcore, but its partner is partially softcore, therefore its C.O.M. will be exchanged.' 
            end if
         end if
      end do
      ...
      do i = 1, nummols
         if (molecule_type(i) == 2) then
            write (6,'(a,i5,a)') '     Molecule ',i,' is completely softcore and skipped for C.O.M..'
         end if
         if (molecule_type(i) == 1) then
            write (6,'(a,i5,a)') '     Molecule ',i,' is partially softcore on this side or the corresponding partner molecule is.'
         end if
...
!===================================================================================================
    ...
    subroutine sc_lngdyn(winv,amass,v,f,sdfac, c_explic, c_implic, istart, iend, nr, dtx,worldrank)

      ! Passed in
      integer istart, iend, nr, worldrank
...
!===================================================================================================
    ...
    subroutine adj_dvdl_stat(dvdl_vdw, dvdl2_vdw, clambda)

      _REAL_ dvdl_vdw, dvdl2_vdw, clambda
      ...
      if ( isProcessV1 ) then
         ...
         dvdl_vdw   = dvdl2_vdw  + weight1 * sc_tot_dvdl_partner + weight0 * sc_tot_dvdl
...
!===================================================================================================
    ...
    subroutine sc_nomix_frc(f,nr3,ener,clambda)
      ...
      dvdl= -ener(23) + weight0 * sc_tot_dvdl
      sc_ener(8) = weight0 * sc_tot_dvdl
      ! include sc_dvdl into vdw and potential energies:
      ener(23) = ener(23) + weight0 * sc_tot_dvdl
      ener(24) = ener(24) + weight0 * sc_tot_dvdl

      ! scale down energies and forces
      ener(1:51) = weight0 * ener(1:51)
...
!===================================================================================================
    ...
    subroutine calc_softcore_ekin(nrp,amass,v,vold)
...
!===================================================================================================
    ...
    subroutine sc_print_energies(channel, energies)
      ...
      if (nsoftcore>0) then
         ...
         write (channel,1200) energies(7),energies(8)
         write (channel,2000)
      end if
      ...
      1200 format (1x,'SC_VDW =    ',f11.4,2x,'SC_DERIV=    ',f11.4)
      2000 format (t2,78('-'),/)
      RETURN
...
!===================================================================================================
    ...
    subroutine cleanup_sc()
      ...
      if (ier_alloc /= 0) call sander_bomb('cleanup_sc[softcore.f]','cant deallocate nsc_partner','')
      ...
      if (ier_alloc /= 0) call sander_bomb('cleanup_sc[softcore.f]','cant deallocate frcmask_sc','')

      if(allocated(frcmask_partner)) deallocate (frcmask_partner, stat=ier_alloc)
      if (ier_alloc /= 0) call sander_bomb('cleanup_sc[softcore.f]','cant deallocate frcmask_partner','')

      if(allocated(dvdl_values)) deallocate (dvdl_values, stat=ier_alloc)
      if (ier_alloc /= 0) call sander_bomb('cleanup_sc[softcore.f]','cant deallocate dvdl_values','')
