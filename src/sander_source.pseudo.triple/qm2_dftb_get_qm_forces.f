...
!!Locals:
   ...
   _REAL_  :: ocmcc,dgrh,xhelp,dgrs,dgr
...
!! ----------
   ...
   !_REAL_, parameter :: deltax = 1.0e-2
...
!! Pointers
   ...
   ! Build the multiplicative factor
   ! SUM_i n_i SUM_mu,nu c_mu,i * c_nu_i
   !
   ! In the derivative of the overlap, there's
   ! also the 'e_i' factor (eigevalue)
   do i = 1,ndim
      do m = 1,ndim
         do n = 1,m-1
                  ...
                  do mj=1,2*lj-1
                     ...
                     do lk = 1,lmax(izpk)
                        do mk = 1,2*lk-1
                           ...
                           dgr = -0.5d0 * dgrs * (shift(k) + shift(j))

                           ! Make sure to multiply by the correct factor.
                           if(mu > nu)then
                              ! b(nu,mu) contains the energy
                              dgrh = dgrh * b(mu,nu)
                              dgrs = dgrs * b(nu,mu)
                           ...
                           grad(i,j) = grad(i,j) + dgrh + dgrs + dgr
