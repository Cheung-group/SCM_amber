...
!     sander is now a subroutine.  See below.
!
...
!
! o  If you add files to files.h, you may need to update the code here to
!        broadcast the names appropriately when -groupfile is not specified.
!
! TODO:
...
#ifdef MPI
   use remd, only : rem, rremd, repnum, maxgroup, numreps
#endif
...
#  include "md.h"
   ...
   if (worldrank == 0 .and. rem > 0 .and. numgroup > maxgroup) then
      write(6,*) 'Error: the number of groups (multiple instances of', &
            ' sander) has been exceeded!'
      write(6,*) 'maxgroup is ', maxgroup
      write(6,*) 'Change maxgroup in remd.f and recompile.'
      call mexit(6,1)
   end if
...
!        endif
   ...
   if (numgroup > 1) then
      ...
      ! Since rem was rebroadcast, check for numgroup>maxgroup again.
      if (worldrank == 0 .and. rem > 0 .and. numgroup > maxgroup) then
         write(6,*) 'Error: the number of groups (multiple instances of', &
                 ' sander) has been exceeded!'
         write(6,*) 'maxgroup is ', maxgroup
      ...
      if (worldrank == 0) then
         write(6, '(a)') ''
         write(6, '(a)') ' Running multisander version of sander amber10'
   ...
   if (rem > 0) then
      ...
      ! DAN ROE: Note that all processes are calling this. 

      call amrset(71277 + 17 * nodeid)

      ! NOTE - AMRSET CALLED AGAIN TO RESET VALUE FOR RESERVOIR
      ! RUNS BASED ON SEED IN SAVEENE FILE
