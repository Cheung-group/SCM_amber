...
do im_new = 1,icount
   ...
   if ( nsc(i) == nsc(j) ) then 
      ...
      denom = 1.0d0 / ( scalpha * ( 1.0d0 - clambda ) + r6 * sigma6(ic) ) !sigma6 is 1/(sigma^6)
      denom2 = denom * denom
      denom3 = denom2 * denom
      ...
      ! -- ti decomp
      if(decpr .and. idecomp > 0) call decpair(3,i,j,weight1*foureps(ic)*(2.0d0*scalpha*denom3 - scalpha*denom2)/(nstlim/ntpr))
