...
#  include "def_time.h"
   ...
   integer l_tmpcrd,l_tmpvel,l_tmpxr,l_tmpdp,l_tmpvel2
   integer istart,inext,ilast,inwat,itmpro
   _REAL_ v(*),xr(*),vold(*)
   integer ntp
   integer ictmplist,iftmplist
   save trial,balance
   save last_numlist
#ifdef MPI
   save listdiffmax
...
subroutine grid_image(natom, verbose,periodic,crd)

   use constants, only : zero, half
   implicit none
   integer, intent(in) ::  natom,verbose,periodic
   _REAL_, optional, dimension(3,natom), intent(in) :: crd
   _REAL_ :: dx,dy,dz
   integer jh
...
!QMMM 
   if ( qmmm_nml%ifqnt ) then
      if ( qmmm_struct%atom_mask(i) ) then ! then current atom is a QM atom
        do qm_temp_count2=1, qmmm_struct%nquant
          exclude(qmmm_nml%iqmatoms(qm_temp_count2))=i ! skip interaction with all other QM atoms
        end do
      end if
   end if
!END QMMM
...
! Qian change, close this function
...
subroutine check_skin(crd,do_list_update)
   ...
   implicit none
   _REAL_  crd(3,*)        ! current atom coordinates, intent(in)
   logical do_list_update  ! true if a list update is needed, intent(out)
#  include "extra.h"
#  include "memory.h"
