...
private  angnrg, at2res, aveint, commass, disnrg, getnat, gendisnrg, jnrg, &
         mrcmf, nmrcms, nmrgrp, parenfind, parscom, parsgen, plnnrg, plptnrg, &
...
subroutine getnat(iatlocal,atnm,name,ipres,nres,iout,ierr)
   ...
   do 10 i = ipres(iatlocal),ipres(iatlocal+1)-1
      if (atnm == name(i)) then
         iatlocal = i
         return
      end if
...
subroutine jnrg  (x         ,f         ,ajval     ,i1        , &
   ...
   dimension ajcoef(3)
#ifdef LES
   parameter (maxatom3=15000)
   dimension drv(maxatom3)
...
#  include "memory.h"
   ...
   if( iflag == 2 ) write( iuse, '(35x,f10.3,1h(,f9.3,1H))') &
         ajnow,57.296*ap
   if( k2 < 0.0 ) then
      e = 0.0
      return
...
subroutine modwt(wtnrg,iwtstp,iwttyp,ichgwt,ishrtb,nstep,temp0, &
      ...
      nttyp,nhb,ncharg,ntypes,rad,wel,radhb,welhb, &
...
subroutine nmrcmf(f,dfr,nmrat,nmrcom,rmstot,rimass,i)
   ...
   do 5 iat = 1,8
      ...
      if (nmrat(iat,i) < 0 .and. nmrat(iat+8,i) < 0) then
         do 10 ip = -nmrat(iat,i),-nmrat(8+iat,i)
            do 20 j = nmrcom(1,ip),nmrcom(2,ip)
               jx = 3*(j-1)
...
subroutine nmrgrp(i,kat,lastpk,iscrth,natom,nmrat,nmrcom,in,iout, &
   ...
   ! and so on up through NMRAT(8,I) and NMR(16,I) if that many groups are defined
...
!end
   ...
   29 do 22 m=1,9
      do 23 n=10,1,-1
         if (redirc(m)(n:n) /= ' ') goto 24
      23 continue
      24 call chklin(aline,redirc(m),n,imat,istrt,ilengt,iout)
      if (imat /= 0) then
...
! comienzo del loop lectura de los restraints indice i
   ...
   do 30 i=1,999999
      ...
      !  Using this new input will override the values in r1-r4 and rk2-rk3.
...
subroutine parsrest(restraint,iat,igrarr,rstwt,name,ipres,nres,iout)
  ...
  !parentype: The type of parentheses used in the current grouping
...
#  include "les.h"
   ...
   if (iat1 >= 0) then
      ...
      ! group of atoms for angle, torsion, and my new planar restraints
         do 24 j=nmrcom(1,jp),nmrcom(2,jp)
            j3 = 3*(j-1)
            ipr = ipr + 1
      ...
      j3 = iat2
      do 35 ip=-iat1,-nmrat(9,indx)
      ! Modified 9/2007 by Matthew Seetin as a consequence of enabling restraints on
      ! group of atoms for angle, torsion, and my new planar restraints
         do 34 i=nmrcom(1,ip),nmrcom(2,ip)
            i3 = 3*(i-1)
            ipr = ipr + 1
            if (ipr > maxpr) goto 500
            xij(1,ipr) = x(i3+1) - x(j3+1)
      ...
      do 48 jp=-iat2,-nmrat(10,indx)
      ! Modified 9/2007 by Matthew Seetin as a consequence of enabling restraints on
      ! group of atoms for angle, torsion, and my new planar restraints
         do 47 j=nmrcom(1,jp),nmrcom(2,jp)
            j3 = 3*(j-1)
            do 45 ip=-iat1,-nmrat(9,indx)
               do 44 i=nmrcom(1,ip),nmrcom(2,ip)
#ifdef LES
                  if( subsp(i) == subsp(j)  .and. &
                        cnum(i) /= cnum(j) ) goto 44
#endif
                  i3 = 3*(i-1)
                  ipr = ipr + 1
                  if (ipr > maxpr) goto 500
...
!      call smorse(NATOM,rk2nmr(2,i),r2nmr(2,i),nmrat(1,i), &
        ...
        implicit none
        integer nori1,nori2,nat1,nat2,natoms,nstep,NTWE
        double precision forcek,fveloc,lambda,          &
        X(3*natoms),F(3*natoms),DT,Ec,rc,dx,dy,dz,      &
        Em,De,Beta,r0,Dem
        nat1=(nori1+3)/3
        nat2=(nori2+3)/3
