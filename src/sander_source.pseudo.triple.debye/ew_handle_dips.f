...
subroutine handle_induced(crd,numatoms,iac,ico,nntypes,charge, &
      cn1,cn2,asol,bsol,eelt,epol,frc,x,ix,ipairs, &
      pol, &
      xr,virvsene,ipres,ibgwat,nres,aveper,aveind,avetot,emtot, &
      dipole_rms, dipiter,dipole_temp,dt,scee,scnb,ntb)

   implicit none
   integer numatoms,iac(*),ico(*),nntypes,ibgwat,nres,ipres(*)
...
#  include "def_time.h"
   ...
   if ( indmeth < 3 .or. initdip == 1 )then
      ...
      do iter = 1,maxiter
         call ewald_force(crd,numatoms,iac,ico,nntypes,charge, &
               cn1,cn2,asol,bsol,eelt,epol,frc,x,ix,ipairs, &
