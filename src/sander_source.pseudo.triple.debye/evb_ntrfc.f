...
#include "dprec.h"
   ...
   subroutine evb_ntrfc ( x, f, ener, mass, ix, ipairs, vel0_nrg_sum )

   use evb_amber, only: xnrg, xq, xf, morsify_initialized, modvdw_initialized 
   use evb_parm,  only: xch_type, nevb, nmorse, nmodvdw 
   ...
   use evb_pimd,  only: evb_begin, evb_end, lpimd_dcrypt, natomPCL, natomPQM, bead_dcrypt &
                      , atomCL_dcrypt, atomQM_dcrypt, PE_slice &
                      , pie, pif, nrg_bead, fpimd &
                      , evb_mat_bead, evb_vec0_bead, vel0_bead &
                      , master_worldrank
   use evb_check, only: evb_pimd_debug
#endif /* LES */
...
#  include "les.h"
   ...
   _REAL_ , intent(inout) :: f(3,natom), ener(*), vel0_nrg_sum
...
#ifdef LES
   integer :: j, k, mm, evb_tag, vel0_tag, nslice, npimd_set 
   integer :: send_status( MPI_STATUS_SIZE,nbead*3) &
!           , receive_status( MPI_STATUS_SIZE,nbead*3) &
            , receive_status( MPI_STATUS_SIZE) &
            , sreq(nbead*3), rreq(nbead*3), ntag &
            , rreq_vec0(nbead)
   ...
   _REAL_  :: evb_vec0_lpimd(nevb,lpimd_size), evb_vec0_tmp(nevb,nbead) &
            ...
            , f0_lpimd(3,natomCL,lpimd_size), f0_tmp(3,natomCL,nbead)
   ...
   integer :: ierr, status(MPI_STATUS_SIZE), rreq1, rreq2, rreq3
   integer :: m, n, nn, nnn, nPE, idx, nrreq 
...
!  +---------------------------------------------------------------------------+
   ...
   if( master ) then
      ...
      if( nmorse > 0 ) then
         ...
         ener(5) = ener(5) + sum( nrg_morsify(:) )
         ener(1) = ener(1) + sum( nrg_morsify(:) )
      endif
      ...
      if( nmodvdw > 0 ) then
         ...
         ener(2) = ener(2) + sum( nrg_vdw(:) )
         ener(1) = ener(1) + sum( nrg_vdw(:) )
      endif
...
!  +---------------------------------------------------------------------------+
   ...
   if( master ) then
      if( trim( adjustl( xch_type ) ) == "dist_gauss" ) then
         ...
         enddo
         ener(1:26) = 0.0d0
         ener(   1) = nrg_diabatic(masterrank+1)
      endif
...
!  call mpi_bcast ( x, natom*3, MPI_DOUBLE_PRECISION, 0, commsander, ierr )
   ...
   if( master ) then 
      ...
      if( trim( adjustl( xch_type ) ) /= "dist_gauss" ) then
         call mpi_allgather ( ener(1), 1, MPI_DOUBLE_PRECISION, xnrg, 1 &
                            , MPI_DOUBLE_PRECISION, commmaster, ierr )
...
!  +---------------------------------------------------------------------------+
      ...
      if( trim( adjustl( xch_type ) ) == "dist_gauss" ) then
         ener(1:26) = 0.0d0
         ener(   1) = evb_Hmat%evb_mat(masterrank+1,masterrank+1)
      endif
   endif 
