...
!Antonios changed here for size-dependence 12/1/2010
        do gamma_index=1,nrsize
        sdfac_as(gamma_index) = sqrt( 4.d0*gammai_as(gamma_index)*boltz2*temp0/dtx)
!   write(72,*)gammai,gammai_as(gamma_index),gamma_index
!   write(72,*)sdfac_as(gamma_index),gamma_index
...
#  else
         rsd = sdfac*sqrt(aamass)
!Antonios changed 11/20/2010 for Brownian dynamics
!Qian change
...
!Qian change,rsd_index should calculated with i3, otherwise it is not corrct in mpi version
...
!Qian add cap for the force
          if(f(i3) .gt. 1000.0) then
          write(84,*)"warning in runmd.f",f(i3),nstep,i3
          f(i3) = 1000.0
          endif
          if(f(i3) .lt. -1000.0) then
          write(84,*)"warning in runmd.f",f(i3),nstep,i3
          f(i3) = -1000.0
          endif
!Qian add end
...
! Qian changes folding condition for protein-protein association
! check distance
          com_x=0.0
          com_y=0.0
          com_z=0.0
! center of mass of target
          DO i_olap = target_init,target_final
              com_x = com_x + x((i_olap-1)*3+1)
              com_y = com_y + x((i_olap-1)*3+2)
              com_z = com_z + x((i_olap-1)*3+3)
          enddo
          temnum = target_final - target_init + 1
          com_x = com_x/temnum
          com_y = com_y/temnum
          com_z = com_z/temnum
...
! move the system
          DO i_olap = 1,nrsize
          x((i_olap-1)*3+1) = x((i_olap-1)*3+1) - com_x + com_x0
          x((i_olap-1)*3+2) = x((i_olap-1)*3+2) - com_y + com_y0
          x((i_olap-1)*3+3) = x((i_olap-1)*3+3) - com_z + com_z0
          enddo
! center of mass of CaM+calcium
          temnum = target_init - 1
          com_x=0.0
          com_y=0.0
          com_z=0.0
          DO i_olap = 1,temnum
              ...
              com_z = com_z + x((i_olap-1)*3+3)
          enddo
          DO i_olap = target_final+1,target_final+4
              com_x = com_x + x((i_olap-1)*3+1)
              com_y = com_y + x((i_olap-1)*3+2)
              com_z = com_z + x((i_olap-1)*3+3)
          enddo
          com_x = com_x/(temnum+4)
          com_y = com_y/(temnum+4)
          com_z = com_z/(temnum+4)

          comdist = (com_x-com_x0)**2+(com_y-com_y0)**2+(com_z-com_z0)**2

          comdist = sqrt(comdist)
          if(comdist > qloop) then
          write(75,*)"escape",comdist,NSTEP
          NSTEP = NSTLIM
!Qian change
          itdump = 1
          ixdump = 1
!Qian change end
          else if(comdist < checkloop) then
! check overlap
! Qian change, check interface Q
        count_olap=0
        Q=0
          !DO i_olap = 1,nsolut
           !DO j_olap = i_olap+2,nsolut
          DO Qmap_count = 1,Qmap_total
          i_olap = Qmap(Qmap_count,1)
          j_olap = Qmap(Qmap_count,2)
           dist=0
             do M_olap=1,3
         ...
         write(75,*)"associate",count_olap,Q,xdummy_olap,xoverlap,temp_olap,comdist,NSTEP
                NSTEP = NSTLIM
!Qian change
          itdump = 1
          ixdump = 1
...
!Qian change
         ...
         if ((mod(total_nstep,ntwe) == 0.and.onstep) .or. (nstep == nstlim)) then
!Qian change end
