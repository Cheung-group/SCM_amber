...
subroutine rgroup(natom,natc,nres,ngrp,ipres,lbres,igraph,isymbl, &
   ...
   character(len=96) :: inucs = 'DA  DC  DG  DT  DA5 DC5 DG5 DT5 DA3 DC3 DG3 DT3 ' // &
   ...
   do 10 n = 1,isrch
      if((idecomp == 0) .and. (jresnm(n) /= iwild).and.(jresnm(n) /= lbres(ires))) goto 10
      if((jgraph(n) /= iwild).and.(jgraph(n) /= igraph(iatom))) goto 10
      if((jtree(n) /= jwild).and.(jtree(n) /= itree(iatom))) goto 10
      if((jsymbl(n) /= jwild).and.(jsymbl(n) /= isymbl(iatom))) goto 10
      if(idecomp > 0) then
         if((jresnm(n) .eq. 'prot').and.(index(ipeps,lbres(ires)) == 0)) goto 10
         if((jresnm(n) .eq. 'nucs').and.(index(inucs,lbres(ires)) == 0)) goto 10
      end if
      iloc = 1
      goto 20
   10 continue
