...
!     --- CALC_HB_DEL --
!----------------------------------------------------------------------------
!     ...A routine to calc the delx,dely and delz for the pairs needed to complete
!     the HB calculation.... A. Samiotakis 6/14/2010
!----------------------------------------------------------------------------
subroutine calc_hb_del(iac,ico,ntypes,charge, &
...
#endif
   implicit none
   character(kind=1,len=13) :: routine="calc_hb_del"
#ifdef MPI
         ...
         do k = ncell_lo,ncell_hi
            ...
            if ( ntot > 0 )then
               if ( mpoltype == 0 )then
                  ...
                  REQUIRE(rstack_ok)
                  REQUIRE(istack_ok)
                  call short_ene_small(i,xk,yk,zk,ipairs(numpack),ntot,nvdw,nhbnd, &
                        ewaldcof,eedtbdns, &
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+ Calculates the delx, dely and delz terms needed for the HB
!  many body terms. Antonios Samiotakis 6/15/10
!-----------------------------------------------------------------------
!     --- SHORT_ENE_small ---
!-----------------------------------------------------------------------
...
subroutine short_ene_small(i,xk,yk,zk,ipairs,ntot,nvdw,nhbnd, &
      ewaldcof,eedtbdns, &
      eed_cub,eed_lin,charge, &
      ntypes,iac,ico,cn1,cn2,asol,bsol,filter_cut, &
...
#     include "ew_directe2.h"
   ...
   return
end subroutine short_ene_small 
