...
module cns_xref
   ...
   ! History:
   ! $Id: xref.f,v 1.8 2008/03/06 20:45:56 rcw Exp $
   ...
   subroutine write_cns_xref_md_energies( exray, r, rfree )
      !
      ! emit energies during molecular dynamics
      !
      !------------------------------------------------------------------
      implicit none
      _REAL_,           intent(in) :: exray
      _REAL_,           intent(in) :: r
      _REAL_,           intent(in) :: rfree

      character(*), parameter :: frmt = &
"(1x,'EXRAY = ', f14.4,2x, 'R     = ',f14.4,2x, 'RFREE     = ',f14.4)"

      if ( is_xref_active() ) then
         write(6, frmt) exray, r, rfree
      end if
   ...
   subroutine write_cns_xref_min_energies( exray, r, rfree, eamber )
      !
...
"(1x,'EXRAY   = ',f13.4,2x,'R       = ',f13.4,2x, 'RFREE      = ',f13.4)"
      ...
      if ( is_xref_active() ) then
         write(6, frmt) exray, r, rfree
         write(6, "(1x,'EAMBER  = ',f13.4)" ) eamber
      end if
