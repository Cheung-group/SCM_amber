...
subroutine qm_ewald_setup(totkq,kmaxqx,kmaxqy,kmaxqz,ksqmaxq,natom,nquant, nlink)
...
!    do i = 1, nquant_nlink
    do i = qmmm_mpi%nquant_nlink_start, qmmm_mpi%nquant_nlink_end
      ...
      do j = 1, npairs
        ...
        call erfcfun(rij*ew_coeff,erfcx)
...
!Requires the Mulliken Charges (or equivalent).
  ...
  use constants, only : INVSQRTPI, zero, one, two
  implicit none
...
!Local
  ...
  _REAL_ :: esfact, qmi(1:3), vec(1:3), kvectmp
...
!  do i =1,nquant_nlink
  do i = qmmm_mpi%nquant_nlink_start, qmmm_mpi%nquant_nlink_end
    ...
    do j = 1,iminus
      ...
      rij = r2*oneRIJ !one/oneRIJ
      call erfcfun(rij*ew_coeff, erfcx)
      qmewald%qmpot(j) = qmewald%qmpot(j) - chg_qm*(one - erfcx)*onerij
    end do
    !i=j
    qmewald%qmpot(i) = qmewald%qmpot(i) + esfact*chg_qm
    iplus = i+1
...
!Local
  ...
  do i = 1, qmmm_struct%nquant_nlink
    ewdpot = (mmpot(i)+half*qmpot(i)) * AU_TO_EV * BOHRS_TO_A
                         !AU_TO_EV*BOHRS_TO_A converts (electrons/angstrom) to (eV/Bohr)
...
!    do i = 1, nquant_nlink
    do i = qmmm_mpi%nquant_nlink_start, qmmm_mpi%nquant_nlink_end
      ...
      do j = 1,qmmm_struct%qm_mm_pairs
        loop_count = loop_count+1
        rij_coeff = qm2_rij_eqns%qmmmrijdata(QMMMRIJ,loop_count)*ew_coeff
        call erfcfun(rij_coeff,erfcx) !(2/3)exp(rij_coeff^3)*INVSQRTPI
        d_erfcx = two*ew_coeff*exp(-rij_coeff*rij_coeff)*INVSQRTPI
