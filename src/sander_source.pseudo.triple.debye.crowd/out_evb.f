...
#include "dprec.h"
   ...
   use evb_parm,  only: nevb, nbias, evb_dyn, ntw_evb, inc_dbonds_RC &
                      , out_RCdot
   use evb_data,  only: evb_Hmat, evb_bias, nrg_frc, nrg_vel0, RCdot &
                      , f_v, F_QI, G_QI
#ifdef LES
   use pimd_vars, only: nbead, itimass
   use evb_pimd,  only: evb_mat_bead, evb_vec0_bead, vel0_bead, nbead_inv
   use miller,    only: dlnQ_dl, i_qi
#endif
...
!  +---------------------------------------------------------------+
            ...
            case( "evb_map" )
               ...
               if( inc_dbonds_RC ) then
                  do n = 1, nbias
                     write(evb_unit,'(A)')
                     write(evb_unit,1000) '  [DBONDS RC]', evb_bias%RC(n), evb_bias%RC(n)
                  enddo
               endif
#endif 
