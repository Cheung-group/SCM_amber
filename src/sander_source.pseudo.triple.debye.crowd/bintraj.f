...
module bintraj
   ...
   public open_binary_files, close_binary_files, write_binary_traj, end_binary_frame
contains
...
#  include "files.h"
 ...
 select case (unit)
      case (MDCRD_UNIT)
         call checkerror(nf90_put_var(crd_ncid, crd_TimeVarID, &
               (/ t /), start = (/ crd_frame /), count = (/ 1 /)), 'write time')
         
         call checkerror(nf90_sync(crd_ncid))

         crd_frame = crd_frame + 1
         ! Sync frame counters if combined trajectory
         if (ntwv == -1) vel_frame = vel_frame + 1
      case (MDVEL_UNIT)
         call checkerror(nf90_put_var(vel_ncid, vel_TimeVarID, &
               (/ t /), start = (/ vel_frame /), count = (/ 1 /)), 'write time')
         
         call checkerror(nf90_sync(vel_ncid))
...
subroutine open_nc_file(ncid,filename,owrite,facc,natom,title,exists,frame,TimeVarID)
   ...
   character(len=80), intent(in) :: filename,title
...
subroutine end_nc_define(ncid)
   ...
   call checkerror(nf90_set_fill(ncid, nf90_nofill, oldMode), "set no fill")
   call checkerror(nf90_enddef(ncid), "end define")

   ! Fill dimension label variables
   call checkerror(nf90_put_var(ncid, SpatialVarID, &
