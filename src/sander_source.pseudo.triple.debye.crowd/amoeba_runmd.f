...
#  include "files.h"
   integer dim1,num_list,n,j,k,nf,ibead
   integer :: ier,iok,ionerr,indicator
...
!------------------------------------------------------------
subroutine AM_RUNMD_write_restrt(natom,t,title,restrt,crd,vel)
   use nblist, only: a,b,c,alpha,beta,gamma
   integer, intent(in) :: natom
   _REAL_,intent(in) :: t
...
#  include "memory.h"
#  include "files.h"
   integer :: m,nstep,nitp,nits,nren
   _REAL_ :: vir(4),ener(51),enert(51),enert2(51), &
             enert_old(51),enert2_old(51),onefac(3),degrees_of_freedom
   ...
   do nstep = 1,nstlim
      ...
      if ( ntwr > 0 )then
         if ( mod(nstep,ntwr)==0 )then
            call AM_RUNMD_write_restrt(natom,t,title,restrt,crd,vel)
         endif
      endif
      if ( nscm > 0 )then
...
! get the forces
   call force(xx,ix,ih,ipairs,crd,frc,ener(23),vir, &
...
subroutine AM_RUNMD_update_enert(nren,ener,enert,enert2)
   ...
   _REAL_,intent(in) :: ener(nren)
   _REAL_,intent(inout) :: enert(nren),enert2(nren)
...
subroutine AM_RUNMD_running_ave(nren,nstep,ntave,onefac,t, &
   ...
   _REAL_,intent(in) :: onefac(3),t,enert(nren),enert2(nren)
   _REAL_,intent(inout) :: enert_old(nren),enert2_old(nren)

   _REAL_ :: enert_tmp(51),enert2_tmp(51)
   integer :: m
   ...
   do m = 1,nren
      ...
      enert_tmp(m) = enert_tmp(m)/ntave
      enert2_tmp(m) = enert2_tmp(m)/ntave - enert_tmp(m)*enert_tmp(m)
...
subroutine AM_RUNMD_overall_ave(nren,nstlim,onefac,t,enert,enert2)
   ...
   do m = 1,nren
      enert(m) = enert(m)/nstlim
      enert2(m) = enert2(m)/nstlim - enert(m)*enert(m)
