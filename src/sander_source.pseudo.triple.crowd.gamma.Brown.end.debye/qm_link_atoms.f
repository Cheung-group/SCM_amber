...
! if link atoms are detected below and qmgb=1 then the program will quit.
  ...
  use qmmm_module, only : qmmm_nml, qmmm_struct,reallocate_iqm_atno_array, reallocate_iqmatoms_array
...
!     of atoms they are between.
  ...
  do i=1,nbona
     ...
     jj = .false.
     do j=1,qmmm_struct%nquant
       if ((ib(i)/3 + 1).eq.qmmm_nml%iqmatoms(j)) then
          kk=j
          ii = .true.
       end if
       if ((jb(i)/3 + 1).eq.qmmm_nml%iqmatoms(j)) then
          kk=j
          jj = .true.
       end if 
     enddo
...
!     We now know how many link atoms there are so re-allocate our qmmm_struct%iqm_atomic_numbers to include them
  ...
  if (qmmm_struct%nlink /= 0 ) then
     call reallocate_iqm_atno_array(qmmm_struct%nquant,qmmm_struct%nquant_nlink)
     ...
     !Also rellocate iqmatoms array to make it nquant_nlink long and fill the link atom
     !section of it with the mm link pair numbers
     !At the same time we will set the mm_link_mask to true for mm link pair atoms
     call reallocate_iqmatoms_array(qmmm_struct%nquant,qmmm_struct%nquant_nlink)
     do i = 1, qmmm_struct%nlink
        imm = qmmm_struct%link_pairs(1,i)
        qmmm_nml%iqmatoms(qmmm_struct%nquant+i) = imm
...
#include "memory.h"
   ...
   do i=1,qmmm_struct%nlink
      ...
      onerij = one/sqrt(rij2) * qmmm_nml%lnk_dis
      offset = offset+1
    ...
    do j = 1, qmmm_struct%nlink
      write(6,'("QMMM:",i6," ",A4," ",i6," ",A4,5F10.3)') &
                qmmm_struct%link_pairs(1,j),atom_type(qmmm_struct%link_pairs(1,j)), &
                 qmmm_nml%iqmatoms(qmmm_struct%link_pairs(2,j)), &
                 atom_type(qmmm_nml%iqmatoms(qmmm_struct%link_pairs(2,j))), &
...
!Allocate mm_link_pair_saved_coords if first call
   if (qmmm_struct%adj_mm_link_pair_crd_first_call) then
     ...
     allocate(qmmm_struct%mm_link_pair_saved_coords(3,qmmm_struct%nlink*ncopy), stat=ier)
     REQUIRE(ier == 0)
   end if
   ...
   do i = 1, qmmm_struct%nlink
     ...
     iq = qmmm_nml%iqmatoms(qmmm_struct%link_pairs(2,i))
     qmx = unimaged_coords(1,iq)
     qmy = unimaged_coords(2,iq)
     qmz = unimaged_coords(3,iq)
     xij = unimaged_coords(1,im) - qmx
