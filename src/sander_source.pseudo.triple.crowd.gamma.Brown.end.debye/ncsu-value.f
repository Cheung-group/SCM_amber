...
!
! sort of C/C++ union (looks sort of brain-damaged in FORTRAN)
!
...
recursive subroutine assign_V_to_V(var, expr)
   ...
   if (associated(var%iptr, target = expr%iptr) .or. &
       associated(var%rptr, target = expr%rptr) .or. &
       associated(var%sptr, target = expr%sptr)) &
      return

   if (associated(var%tail, target = expr%tail)) then
