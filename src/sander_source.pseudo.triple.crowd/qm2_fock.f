...
!Local
   ...
   do ii=2,qmmm_struct%nquant_nlink
      ...
      do JJ=jstart,jend
         JA=orb_loc(1,JJ)
         JB=orb_loc(2,JJ)
         IF(IB /= IA .AND. JA /= JB) THEN ! HEAVY-ATOM  - HEAVY-ATOM
            ...
            KK=KK+100
         ELSEIF(IA /= IB)THEN ! LIGHT-ATOM  - HEAVY-ATOM
            !   COULOMB TERMS
            SUMDIA=0.D0
            SUMOFF=0.D0
            ...
            KK=KK+10
         ELSEIF(JA /= JB)THEN ! HEAVY-ATOM - LIGHT-ATOM
            !   COULOMB TERMS 
            SUMDIA=0.D0
            SUMOFF=0.D0
...
! dimension 36 = max of 8*(8+1)/2 = 4 orbs with 4 orbs - S,3P with S,3P
   _REAL_, intent(inout) :: F(36)
   _REAL_, intent(in) :: ptot(36)
   _REAL_, intent(in) :: W(100)
   integer, intent(in) :: orb_loc(2,2)

!Local
   integer JINDEX(256)
   _REAL_ PK(16),fock2_ptot2_1(16),fock2_ptot2_2(16)
