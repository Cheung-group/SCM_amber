...
#  include "ew_mpole.h"
   ...
   ene(3) = 0.d0
   ene(1) = ener(23)
   ene(42) = tmass / (0.602204d0*volume)
   ene(10) = volume
   ...
   if ( dumpfrc == 1 )then
      call force_dump(natom,xx,ix,ih,ipairs,x,f, &
            ...
            cn1,cn2,xx(lfixdip),nttyp,xx(linddip),xx(lsind),qsetup)
   end if
   if ( rmsfrc == 1 )then
      call rms_check(natom,xx,ix,ih,ipairs,x,f, &
            ...
            xx(l15),cn1,cn2,xx(lfixdip),nttyp, &
...
subroutine rms_check(natom,xx,ix,ih,ipairs,x,f, &
      ...
      fixdip,nttyp,inddip,sind,qsetup)
...
#  include "flocntrl.h"
   ...
   integer ipairs(*),ix(*)
   character(len=4) ih(*)
   integer j,k,natom,jmax,matom,jmin,nttyp
   ...
   _REAL_ maxrms,rms,ene(52),virial(4)
   ...
   if ( master )then
      ...
      write(6,*)'-----------------------------------------'
      call compare(ener(23),tvec(1),'Potential energy:      ')
      call compare(ener(24),tvec(2),'Nonbond energy:        ')
      call compare(ener(25),tvec(3),'Electrostatic energy:  ')
      call compare(ener(27),tvec(5),'Bond energy:           ')
      call compare(ener(28),tvec(6),'Angle energy:          ')
      call compare(ener(29),tvec(7),'Dihedral energy:       ')
      call compare(ener(30),tvec(8),'1-4VDW energy:         ')
      call compare(ener(31),tvec(9),'1-4EE energy:          ')
      call compare(ener(32),tvec(10),'Constraint energy:     ')
      call compare(ener(33),tvec(11),'Polarization energy:   ')
      write(6,*)'-----------------------------------------'
...
subroutine force_dump(natom,xx,ix,ih,ipairs,x,f, &
      ener,scg,scn1,scn2,sdip,cg,cn1,cn2,fixdip,nttyp, &
      inddip,sind,qsetup)
  use ew_recip,only:frcx
   implicit none
#  include "ew_frc.h"
...
#  include "flocntrl.h"
   ...
   integer ipairs(*),ix(*)
   character(len=4) ih(*)
   integer j,natom,nttyp,kk
   _REAL_ inddip(3,*),sind(3,*)
   integer  sdo_dir,sdo_rec,sdo_adj,sdo_self, &
         sdo_bond,sdo_angle,sdo_ephi
   integer  szerochg,szerovdw,szerodip,count
   _REAL_ ene(52),virial(4)
   logical qsetup
   ...
   if ( master )then
      count = count + 1
      write(30,205)count
      205 format(1x,i2,' Van Der Wals Force')
      do j = 1,natom
         write(30,301)f(1,j),f(2,j),f(3,j)
      end do
   end if
...
#define MPI_DOUBLE_PRECISION MPI_REAL8
   ...
   _REAL_ f(*),ftmp(*)
   integer i
   call mpi_reduce(f, ftmp, 3*natom, &
