...
!!jtc ========================= PUPIL INTERFACE =========================
   ...
   logical newstyle
! Pengzhi 9/20/14
 INTEGER, ALLOCATABLE, DIMENSION(:) :: Qmap_tmp
#  include "files.h"
#  include "memory.h"
#  include "nmr.h"
#  include "box.h"
...
! Anotnios comments out for large size
...
! Qian add end
! Reading loop.inp
  open(82,file='associate.inp')
  read(82,*) checkloop,qloop,target_init,target_final,com_x0,com_y0,com_z0
          !targetcoor = com_x0**2+com_y0**2+com_z0**2
! Pengzhi 9/20/14
!  write(80,*) checkloop,qloop,target_init,target_final,com_x0,com_y0,com_z0
  close(82)
! Reading Qmap
  open(82,file='Qmap.inp')
  read(82,*) Qmap_total
! Pengzhi 9/20/14
!  write(80,*) Qmap_total
  do j=1,Qmap_total
  read(82,*) Qmap(j,1),Qmap(j,2)
  enddo
  close(82)

! Pengzhi 9/20/14
!  do j=1,Qmap_total
!  write(80,*)Qmap(j,1),Qmap(j,2)
!  enddo
! Qian add end
...
! From associate.inp
 CALL MPI_Bcast( checkloop, 1, MPI_DOUBLE_PRECISION, 0, commsander, ierr )
 CALL MPI_Bcast( qloop,     1, MPI_DOUBLE_PRECISION, 0, commsander, ierr )
 CALL MPI_Bcast( com_x0,    1, MPI_DOUBLE_PRECISION, 0, commsander, ierr )
 CALL MPI_Bcast( com_y0,    1, MPI_DOUBLE_PRECISION, 0, commsander, ierr )
 CALL MPI_Bcast( com_z0,    1, MPI_DOUBLE_PRECISION, 0, commsander, ierr )
 CALL MPI_Bcast( target_init,  1, MPI_INTEGER, 0, commsander, ierr )
 CALL MPI_Bcast( target_final, 1, MPI_INTEGER, 0, commsander, ierr )
! From Qmap.inp
 CALL MPI_Bcast( Qmap_total, 1, MPI_INTEGER, 0, commsander, ierr )
 ALLOCATE( Qmap_tmp(1:2*Qmap_total) )
 DO j=1,Qmap_total
  Qmap_tmp(j)            = Qmap(j,1)
  Qmap_tmp(j+Qmap_total) = Qmap(j,2)
 END DO
 CALL MPI_Bcast( Qmap_tmp, 2*Qmap_total, MPI_INTEGER, 0, commsander, ierr )
 DO j=1,Qmap_total
  Qmap(j,1) = Qmap_tmp(j)
  Qmap(j,2) = Qmap_tmp(j+Qmap_total)
 END DO
 DEALLOCATE( Qmap_tmp )
! From triple.inp
! Pengzhi 10/30/15
